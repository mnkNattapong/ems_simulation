function [f,t] = EMS1_plot(sol,is_save,save_path)
    ...
    % Calculate expense of with and without EMS
    expense = -min(0,sol.Pnet)*resolution_in_hour.*PARAM.Buy_rate;
    expense_noems = -min(0,PARAM.PV-PARAM.PL)*resolution_in_hour.*PARAM.Buy_rate;    
    ...
    % fig(3,2): Cumulative expense with EMS
    nexttile
    ...
    % fig(4,2): Cumulative expense without EMS 
    nexttile
    ...
    if is_save == 1
        print(f,strcat(save_path,'/EMS1/png/8_plot_',PARAM.TOU_CHOICE,'_',num2str(PARAM.Resolution),'min_',PARAM.start_date,'-dpng'))
        print(f,strcat(save_path,'/EMS1/eps/8_plot_',PARAM.TOU_CHOICE,'_',num2str(PARAM.Resolution),'min_',PARAM.start_date,'-deps'))
    end
